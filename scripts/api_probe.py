...
then call Quantum ELD for one unit and print that JSON too.

By default it pulls one small page from Motive's v1 vehicle_locations feed,
picks the first unit number it sees, and looks that unit up on Quantum — so a
single run exercises both APIs with real, related data. Pass --unit to force a
specific Quantum lookup, or --version v3 to probe Motive's v3 feed.
...
async def call_quantum(session: aiohttp.ClientSession, unit: str) -> dict:
    if not config.QUANTUM_TOKEN:
        print("QUANTUM_TOKEN is not set in .env")
        return {}

    key = quantum_key(unit)
    url = f"{config.QUANTUM_BASE_URL.rstrip('/')}/vehicles/{key}"
    headers = {
        "accept": "*/*",
...
async def run(version: str, size: int, unit: str) -> int:
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as s:
        ...
        # 2) Quantum for a chosen unit (explicit --unit, else first from Motive).
        target = unit or _first_unit(motive)
        if not target:
            print("\nNo unit number to look up on Quantum "
                  "(Motive returned none and --unit was not given).")
            return 1
        await call_quantum(s, target)
    return 0


def main() -> None:
    p = argparse.ArgumentParser(description="Probe Motive then Quantum.")
    ...
    p.add_argument("--unit", default="",
                   help="Quantum unit to look up (default: first from Motive)")
    args = p.parse_args()
